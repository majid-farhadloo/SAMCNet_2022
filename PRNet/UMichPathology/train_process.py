...
def train_proc(args, train_loader, test_loader, model, optimizer):
    ...
    for epoch_idx in range(args.epochs):
        ...
        for batch_idx, (xs, ts, ls) in enumerate(train_loader):
            ...
            batch_pred_ys, batch_pred_prs, batch_true_ys = get_batch_preds(xs, ts, ls)
            if flag == 0:
...
def get_batch_preds(model, args, xs, ts, ls):
    '''
    ...
    return batch_pred_ys, batch_pred_prs, batch_true_ys
