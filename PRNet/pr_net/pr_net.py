...
from PRNet.pr_net.point_attention_layer import PointAttentionLayer
from PRNet.pr_net.pointpair_representation_layer import PointpairRepresentationLayer
from PRNet.pr_net.binary_context_layer import BinaryContextLayer
...
class PRNet(th_nn.Module):
    ...
    def forward(self, raw_data, neighborhood_tensor, core_point_idxs):
        ...
        for feature_idx in range(self.feature_type_count):
            binary_context[:, feature_idx, :] = self.activate(self.linear(raw_binary_context[:, feature_idx, :])) #adj matrix of each pointpair feature
